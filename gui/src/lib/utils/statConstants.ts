...
export const STAT_LABELS: Record<StatKey, string> = {
  hp: 'HP', fo_atk: 'Fo.Atk', fo_def: 'Fo.Def',
...
export const STAT_MAX: Record<StatKey, number> = {
  hp: 370, fo_atk: 250, fo_def: 290, sp_atk: 235, sp_def: 290, spd: 230,
};

export const STAT_ORDER: StatKey[] = ['hp', 'fo_atk', 'fo_def', 'sp_atk', 'sp_def', 'spd'];

export const BST_MAX = 650;

export const VARIANT_LABELS: Record<'normal' | 'alt_color' | 'alt_costume' | 'wedding', string> = {
  normal: 'Normal', alt_color: 'Color', alt_costume: 'Costume', wedding: 'Wedding',
};

export function statBarSegment(min: number, max: number, scaleMax: number) {
  const leftPct = Math.min(100, (min / scaleMax) * 100);
  const widthPct = Math.max(0, Math.min(100 - leftPct, ((max - min) / scaleMax) * 100));
