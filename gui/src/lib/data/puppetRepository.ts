...
export async function fetchPuppetDetail(
  ...
  const rows = await db.select<PuppetRow[]>('SELECT * FROM puppets WHERE rowid = $1', [rowid]);
  ...
  const [sibs, related1, related2, locations, learnset, skillCards, altForms, abilities] =
    await Promise.all([
      puppet.imagename
      ...
      db.select<AbilityInfo[]>('SELECT name, effect FROM abilities WHERE name IN ($1, $2)', [
        puppet.ability1 ?? '',
        puppet.ability2 ?? '',
      ]),
    ]);
  ...
  const itemCategories = await fetchItemCategoriesForDrops(db, puppet.low_drops, puppet.high_drops);
...
async function fetchItemCategoriesForDrops(
  ...
  const names = [
    ...new Set([...parseDrops(lowDrops), ...parseDrops(highDrops)].map((d) => d.name)),
  ];
  if (names.length === 0) return {};

  const placeholders = names.map((_, i) => `$${i + 1}`).join(', ');
  const itemRows = await db.select<{ name: string; category: string | null }[]>(
