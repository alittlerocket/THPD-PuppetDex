...
const VARIANT_KEYS: SpriteVariant[] = ['normal', 'alt_color', 'alt_costume', 'wedding'];
...
export class PuppetDetailViewModel {
  ...
  get baseStatRanges(): Record<string, StatRange> {
    return this.puppet?.stat_ranges ? JSON.parse(this.puppet.stat_ranges) : {};
  }
  ...
  get displaySprite(): string | null {
    if (this.activeFormIndex === -1) {
      return this.puppet ? variantSprite(this.puppet, this.spriteVariant) : null;
    }
  ...
  get relatedFormsUseType(): boolean {
    ...
    const imagenames = new Set([this.puppet.imagename, ...this.relatedForms.map((f) => f.imagename)]);
  ...
  relatedFormLabel(form: { imagename: string | null; type1: string | null }): string {
