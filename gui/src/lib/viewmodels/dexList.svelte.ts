...
import { fetchPuppetList, fetchFilterOptions, type PuppetSort } from '$lib/data/puppetRepository';
import { loadDexPreferences, saveDexPreferences } from '$lib/data/dexPreferences';
import { loadSettings } from '$lib/data/settingsStore';
import { ALL_ON_ONE_PAGE, defaultSettings } from '$lib/types/settings';
import { activeStatColumns, emptyFilters, type PuppetFilters } from '$lib/types/filters';
...
export class DexListViewModel {
   ...
   */
  currentPage: number = $derived(Math.min(Math.max(1, this.#page), this.pageCount));
  ...
  #activeStats: StatColumn[] = $derived(activeStatColumns(this.appliedFilters));

  statColumns: { key: StatColumn; label: string }[] = $derived(
    this.#activeStats.map((key) => ({ key, label: STAT_COLUMN_LABELS[key] }))
  );
    ...
    for (const puppet of this.puppets) {
      byRow.set(
        ...
        moves.map((name, i) => {
          ...
          if (lvl) parts.push(/^\d+$/.test(lvl) ? `Lv ${lvl}` : 'Start');
          if (sc) parts.push(`SC ${sc}`);
          return { name, source: parts.length ? parts.join(' · ') : '—' };
        })
      );
    }
    return byRow;
