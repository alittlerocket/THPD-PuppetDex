			...
			<div class="ref-card">
				<div class="ref-card-header">
					{#if skill.type}
						<span
							class="ref-badge"
							style="background:{typeColor(skill.type)}"
							>{skill.type}</span
						>
					{/if}
					{#if skill.category}<span
							class="ref-badge"
							style="background:#444">{skill.category}</span
						>{/if}
					{#if skill.class}<span
							class="ref-badge"
							style="background:#333">{skill.class}</span
						>{/if}
					{#if skill.is_mod}<span class="ref-mod-tag"
							>{skill.mod_tab ?? 'Modded'}</span
						>{/if}
				</div>
				{#if skill.jp_name}<div class="ref-jp-name">
						{skill.jp_name}
					</div>{/if}
