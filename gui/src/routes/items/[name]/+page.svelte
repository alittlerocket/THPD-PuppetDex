  ...
  {:else if vm.item}
    <div class="ref-card">
      <div class="ref-card-header">
        {#if vm.item.category}
          <span class="ref-badge" style="background:{categoryColor(vm.item.category)}">{vm.item.category}</span>
        {/if}
      </div>
      {#if vm.item.jp_name}<div class="ref-jp-name">{vm.item.jp_name}</div>{/if}
