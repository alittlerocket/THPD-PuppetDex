<script lang="ts">
  ...
  import { STAT_LABELS, STAT_MAX, STAT_ORDER, BST_MAX, VARIANT_LABELS, statBarSegment } from '$lib/utils/statConstants';
      ...
      <div class="detail-left">
        {#if vm.altForms.length > 0}
          <div class="alt-form-toggle">
            <button class="style-pill" class:active={vm.activeFormIndex === -1} onclick={() => vm.setActiveForm(-1)}>
              Base
            </button>
            {#each vm.altForms as form, i}
              <button class="style-pill" class:active={vm.activeFormIndex === i} onclick={() => vm.setActiveForm(i)}>
        ...
        <div class="detail-sprite-frame">
          {#if vm.displaySprite}
            <SpriteReveal src="/sprites/{vm.displaySprite}" alt={vm.puppet.name} />
          {/if}
        </div>
        ...
        <div class="detail-types">
          {#if vm.puppet.type1}
            <span class="detail-type-badge" style="background:{typeColor(vm.puppet.type1)}">{vm.puppet.type1}</span>
          {/if}
          {#if vm.puppet.type2}
            <span class="detail-type-badge" style="background:{typeColor(vm.puppet.type2)}">{vm.puppet.type2}</span>
          {/if}
        </div>
        ...
        <div class="meta-row">
          ...
          {/if}
          <div class="meta-value">{vm.puppet.is_mod ? 'True' : 'False'}</div>
        </div>
          ...
          <div class="abilities-list">
            {#if vm.puppet.ability1}
              <div class="ability-block">
                <span class="ability-tag">{vm.puppet.ability1}</span>
                {#if vm.abilityInfo[vm.puppet.ability1]}
                  <p class="ability-effect">{vm.abilityInfo[vm.puppet.ability1]}</p>
                {/if}
              </div>
            {/if}
            {#if vm.puppet.ability2}
              <div class="ability-block">
                ...
                {#if vm.abilityInfo[vm.puppet.ability2]}
                  <p class="ability-effect">{vm.abilityInfo[vm.puppet.ability2]}</p>
                {/if}
              </div>
            {/if}
          </div>
      ...
      <div class="detail-right">
        <div>
          <div class="section-title">
            Base Stats{#if vm.activeFormIndex !== -1} — {vm.altForms[vm.activeFormIndex]?.form_name}{/if}
          </div>
          ...
          {#each STAT_ORDER as stat}
            {#if vm.displayStatRanges[stat]}
              ...
              {@const seg = statBarSegment(range.min, range.max, STAT_MAX[stat])}
              {@const base = vm.activeFormIndex === -1 ? vm.puppet[stat] : null}
              <div class="stat-bar-row">
                <span class="stat-bar-label">{STAT_LABELS[stat]}</span>
                <span class="stat-bar-base">{base ?? '—'}</span>
                <div class="stat-bar-track">
                  <div class="stat-bar-fill" style="margin-left:{seg.leftPct}%; width:{seg.widthPct}%"></div>
                </div>
                <span class="stat-bar-value">{range.min}–{range.max}</span>
              </div>
            {/if}
          {/each}
          {#if vm.activeFormIndex === -1 && vm.puppet.bst}
            {@const bstPct = Math.min(100, (vm.puppet.bst / BST_MAX) * 100)}
            <div class="stat-bar-row">
              <span class="stat-bar-label">BST</span>
              <span class="stat-bar-base">{vm.puppet.bst}</span>
              <div class="stat-bar-track">
                <div class="stat-bar-fill" style="width:{bstPct}%"></div>
              </div>
              ...
              {#if vm.lowDrops.length > 0}
                <div class="drops-line">
                  ...
                  {#each vm.lowDrops as d}
                    <a
                      class="item-chip"
                      href="/items/{encodeURIComponent(d.name)}"
                      style="background:{categoryColor(vm.itemCategories[d.name])}"
                    >
                      {d.percent} {d.name}
                    </a>
                  {/each}
      ...
      <div class="detail-section">
        ...
        <div class="table-scroll">
          <table class="data-table">
            <thead>
              <tr><th>Location</th><th>Level Range</th><th>Encounter Rate</th></tr>
            </thead>
            <tbody>
      ...
      <div class="detail-section">
        ...
        <div class="table-scroll">
          <table class="data-table">
            <thead>
              <tr>
                <th>Lv</th><th>Move</th><th>Type</th><th>Category</th><th>Class</th>
                <th>Power</th><th>Acc</th><th>Max SP</th><th>Priority</th><th>PP</th>
              </tr>
            </thead>
            <tbody>
              {#each vm.learnset as mv}
                <tr>
                  <td>{mv.level ?? '—'}</td>
                  <td><a class="move-link" href="/skills/{encodeURIComponent(mv.name)}">{mv.name}</a></td>
                  <td>
                    {#if mv.type}
                      <span class="table-type-badge" style="background:{typeColor(mv.type)}">{mv.type}</span>
                    {:else}—{/if}
                  </td>
                  <td>{mv.category ?? '—'}</td>
                  <td>{mv.class ?? '—'}</td>
      ...
      <div class="detail-section">
        ...
        <div class="table-scroll">
          <table class="data-table">
            <thead>
              <tr>
                <th>SC</th><th>Move</th><th>Type</th><th>Category</th><th>Class</th>
                <th>Power</th><th>Acc</th><th>Max SP</th><th>Priority</th>
              </tr>
            </thead>
            <tbody>
              {#each vm.skillCards as sc}
                <tr>
                  <td>{sc.sc ?? '—'}</td>
                  <td><a class="move-link" href="/skills/{encodeURIComponent(sc.name)}">{sc.name}</a></td>
                  <td>
                    {#if sc.type}
                      <span class="table-type-badge" style="background:{typeColor(sc.type)}">{sc.type}</span>
                    {:else}—{/if}
                  </td>
                  <td>{sc.category ?? '—'}</td>
                  <td>{sc.class ?? '—'}</td>
